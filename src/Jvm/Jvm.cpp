...
// tested, apparently works??? for print hello world
void Jvm::ldc_w(){
    std::cout<<"!ldc_w\n";
    u4 value;
    u2 index = GetIndex2();
...
// todo implement throw out of bounds exception
...
void Jvm::fstore_1(){
    std::cout<<"!fstore_1\n";
    // u1 index = (*CurrentCode->code)[pc++]; // todo delete all these from stores
    U4ToType value;
...
//!k
// todo fix these stores casting and that i2b
void Jvm::dstore_0(){
    std::cout<<"!dstore_0\n";
    Cat2Value value;
    value.AsDouble = popU8FromOpStack();
...
void Jvm::f2d(){
    ...
    value.AsDouble = static_cast<float>(CurrentFrame->OperandStack->Pop());
    pushU8ToOpStack(value.HighBytes, value.LowBytes);
...
void Jvm::d2f(){
    ...
    if(std::isnan(valDouble.AsDouble)){
        ...
        valueFloat = static_cast<float>(valDouble.AsDouble);
    }

    CurrentFrame->OperandStack->push(static_cast<u4>(valueFloat));
...
void Jvm::ifne(){    
    ...
    if(value != 0){
         (*CurrentCode->code)[pc] += offset - 2; // todo this offset to all
    }
...
void Jvm::lookupswitch(){
    ...
    int32_t npairs = read_u4(code, pc);

    std::map<int32_t, int32_t> matchOffsetPairs;
    for (int32_t i = 0; i < npairs; ++i) {
        int32_t match = read_u4(code, pc);
        int32_t offset = read_u4(code, pc);
